...
Following Wang et al.'s approach~\cite{wang2016automatically}, we extract a sequence of AST node tokens from source code files. We consider three different groups of AST node tokens: 1) method invocations and class instance creations, 2) method declarations, type declarations, and enum declarations, 3) control-flow like \textbf{for}, \textbf{if}, etc. 
...
However, in contrast to Wang et al.'s approach that weights the extracted AST tokens equally, we assign weights to the tokens using a term frequency--inverse document frequency (TF-IDF) scheme~\cite{manning2008introduction}. TF refers to the number of times a token appears in a source code file. IDF refers to the reciprocal of the proportion of source code files that contains a token. TF-IDF of a token is a multiplication of its TF and IDF. The resultant sequence of AST tokens are weighted by their TF-IDF values. 

Our proposed approach (DDA) takes the resultant sequence of AST tokens as input vector. Since our input vectors may have different lengths, we append 0 to the input vectors to normalize the length to the length of the longest vector. Note that this representation preserves the program structure since the order of tokens are unchanged. 
...
\label{sec:semi}
In this section, we describe our DDA approach for defect prediction, which aims to detect source code files that may potentially contain bugs. Firstly, let $\mathcal{X}=\{x_1, \dots, x_i, \dots, x_n\}$ denotes a set of source code files in a software project and $\mathcal{Y}=\{y_1, \dots, y_i, \dots, y_n\}$ represents a set of labels for the source code files, where $n$ is the number of source code files in the project. A source code file is labelled 1 if it contains a bug ($y_{i}=1$); otherwise, it is labelled as 0 ($y_i = 0$). 
...
In defect prediction tasks, oftentimes there are only a handful of source files that contain bugs, while the other source files are clean (i.e., bug-free)~\cite{khoshgoftaar2010attribute}. As such, we can expect to see a highly-skewed (imbalanced) distribution of class labels (i.e., buggy vs clean). This imposes difficulties for gradient-based learning approaches, making them more biased towards the majority class (i.e., the class with more data instances). As such, class imbalance learning mechanisms have been helpful to tackle defect prediction problems~\cite{wang2013using}. 

In a similar vein, we develop a simple alternating (random) sampling strategy \cite{Le2015} when training DDA. In a nutshell, we divide the training data into two sets, i.e., a buggy set and a clean set. We then perform an Adam update step by taking a randomly-selected buggy and clean sample in an alternating manner. That is, in update step $i$, we input a buggy sample to DDA whereas, in update step $i+1$, we input a clean sample to DDA, and so on. Effectively, this renders a balanced (bootstrapped) training data for DDA, which would help mitigate the bias from the majority (i.e., clean) class. 
...
In this work, we use a DDA architecture with one hidden layer for each function $f$, $g$, and $h$. We configure the DDA model as follows: the number of neurons (nodes) in the hidden layer of $f$ and $h$ is set to 1000~\cite{vincent2010stacked}, while that of $g$ is 50. Also, the number of neurons in the embedding layer is set to 100, which is similar to~\cite{wang2016automatically}. Meanwhile, the regularization parameter $\lambda$ is set to $0.01$, and the reconstruction parameter $\beta$ is chosen via cross validation on the training data. Finally, we use early stopping~\cite{yao2007early} to reduce the training time of the DDA model.
