...
Following Wang et al.'s approach~\cite{wang2016automatically}, we extract a sequence of AST node tokens from source code files. There are three different types of AST nodes are collected as tokens: 1) nodes of method invocations and class instance creations, 2) method declarations, type declarations, and enum declarations, 3) control-flow nodes like \textbf{for}, \textbf{if}, etc. 
...
Our proposed approach (DDA) takes the sequence of AST tokens, which are assigned TF-IDF values, as input vector, and the lengths of the sequence for each source file must be the same. Since our input vectors may have different lengths, we append 0 to the input vectors to make all the length consistent and equal to the length of the longest vector. Through this generating process, the sequence of AST tokens are represented as input vector. Moreover, we also preserve the program structure information of source code since the order of tokens remains unchanged. 
